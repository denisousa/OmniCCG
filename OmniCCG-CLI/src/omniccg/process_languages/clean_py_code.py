...
class SupernovaSanitizer(ast.NodeTransformer):
    """
    Extreme sanitization for NiCad/TXL.
    Removes Generics, DictComps, Decorators, Async, TypeHints and normalizes arguments.
    """
    
    def visit_FunctionDef(self, node):
        node.returns = None
        node.decorator_list = [] # Remove decorators to avoid indentation errors
        
        # Flatten arguments (PosOnly, KwOnly -> normal Args)
    ...
    def visit_AsyncFunctionDef(self, node):
        # Transform async def into a normal def
    ...
    def visit_ClassDef(self, node):
        node.decorator_list = []
        node.keywords = [] # Remove class keywords (e.g.: metaclass=X)
        
        # FIX GENERICS IN INHERITANCE
        # class A(Generic[T]): -> class A(Generic):
        new_bases = []
        for base in node.bases:
            if isinstance(base, ast.Subscript):
                # If the base is Generic[T], take only the Generic (value)
    ...
    def visit_With(self, node):
        # Explode 'with A, B' into 'with A: with B:'
        if len(node.items) > 1:
    ...
    def visit_Call(self, node):
        # SPECIAL ARGUMENTS FIX (__base__)
        # The parser fails on keywords starting with __
        for kw in node.keywords:
            if kw.arg and kw.arg.startswith("__"):
                kw.arg = kw.arg.replace("__", "") # __base__ -> base
        
        # Remove Unpacking in Call (func(*args, **kwargs))
        # Transform into simple positional arguments to keep the flow
        new_args = []
        ...
        # Remove Keyword Unpacking (**kwargs)
        # Transform the kwargs value into an extra positional argument
        new_keywords = []
        for kw in node.keywords:
            if kw.arg is None: # It's a **kwargs
    ...
    def visit_DictComp(self, node):
        ...
        # TXL understands ListComp, but not DictComp
...
def clean_file(filepath):
    try:
        ...
        try:
            ...
            print(f"[SKIP] Native SyntaxError: {filepath} - {e}")
            return False
...
def process_directory_py(directory):
    count = 0
    errors = 0
    print(f"Starting cleaning in: {directory}")
